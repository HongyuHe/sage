...
rebuild_native_binaries() {
  log "Rebuilding native binaries with generic x86_64 flags"
  pushd "${SAGE_RL_DIR}" >/dev/null

  g++ -O2 -march=x86-64 -mtune=generic -pthread src/mem-manager.cc src/flow.cc -o mem-manager
  g++ -O2 -march=x86-64 -mtune=generic -pthread src/sage.cc src/flow.cc -o sage
  g++ -O2 -march=x86-64 -mtune=generic src/client.c -o client
...
sanity_checks() {
  ...
  command -v mm-link mm-delay mm-loss >/dev/null
