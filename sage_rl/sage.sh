...
for env_name in \
    ...
    SAGE_SHIELD_RULES_FILE \
    SAGE_SHIELD_ACTION_DELTA \
    SAGE_SHIELD_CONSECUTIVE_RISK \
    SAGE_SHIELD_COOLDOWN_STEPS \
    SAGE_SHIELD_LOG_PATH \
    SAGE_CONTROLLER_TIMING_LOG_ENABLED \
    SAGE_CONTROLLER_TIMING_LOG_PATH
do
    env_value="${!env_name}"
    if [ -n "$env_value" ]
    then
